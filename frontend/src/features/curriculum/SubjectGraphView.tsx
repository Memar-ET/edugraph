...
// Shared Neo4j knowledge-graph renderer: used standalone by
// CurriculumGraphPage (its own route + AppShell) and embedded directly
// inside the Ministry subject detail view, so "view the graph" is one
// less click for a ministry_admin browsing a subject's full detail.
export function SubjectGraphView({ code, height = 'h-[calc(100vh-260px)] min-h-[500px]' }: { code: string; height?: string }) {
  ...
  if (graphQuery.isError) {
    return <Banner tone="error">{apiErrorMessage(graphQuery.error, `Could not load the graph for ${code}.`)}</Banner>
  }
  if (!graphQuery.data) return null
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`${height} rounded-lg border border-gray-200 bg-gray-50`}>
