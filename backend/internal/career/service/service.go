...
// GenerateMatches asks the ai-service to score every career path against
// the caller's own per-subject performance, then persists the results.
// userID is the JWT subject, never a client-supplied studentID -- see
// repo.StudentIDByUserID's doc comment for why (this used to take a raw
// studentID straight from the URL, an IDOR letting any authenticated
// account generate or read matches for any other student).
func (s *Service) GenerateMatches(ctx context.Context, userID string) ([]dto.MatchResponse, error) {
	...
	return s.matchesForStudent(ctx, studentID)
}

// Matches returns the caller's own generated career matches. Same
// userID-not-studentID rationale as GenerateMatches above.
func (s *Service) Matches(ctx context.Context, userID string) ([]dto.MatchResponse, error) {
	studentID, err := s.studentIDForUser(ctx, userID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) matchesForStudent(ctx context.Context, studentID string) ([]dto.MatchResponse, error) {
...
func (s *Service) studentIDForUser(ctx context.Context, userID string) (string, error) {
	studentID, err := s.repo.StudentIDByUserID(ctx, userID)
	if errors.Is(err, repository.ErrNotFound) {
		return "", apperrors.Forbidden("no student profile for this account")
	}
	if err != nil {
