# Graph Report - edugraph  (2026-07-17)

## Corpus Check
- 290 files · ~67,378 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1600 nodes · 2561 edges · 320 communities (218 shown, 102 thin omitted)
...
## Communities (320 total, 102 thin omitted)
...
Cohesion: 0.08
Nodes (27): CareerMatchRequest, CareerMatchResult, Client, Time, Context, Repository, Service, New() (+19 more)
...
Cohesion: 0.10
Nodes (26): Time, UUID, ApproveResponse, Context, DriverWithContext, JobStatus, ParsedStructurePayload, Pool (+18 more)

### Community 6 - "Parser Docx Extractor"
Cohesion: 0.09
Nodes (47): _build_units_topics_clos(), _extract_legacy(), extract_structure(), _group_clos_into_topics(), _heading_level(), _iter_block_items(), DOCX counterpart to extractor.py.  Word documents almost always carry proper par, Dict-based equivalent of extractor.py's _group_clos_into_topics --     groups CL (+39 more)
...
Cohesion: 0.21
Nodes (12): Handler, Request, ResponseWriter, Service, New(), sniffCurriculumMime(), writeServiceError(), Request (+4 more)
...
Cohesion: 0.13
Nodes (15): eslint, devDependencies, eslint, @playwright/test, postcss, @typescript-eslint/eslint-plugin, @vitejs/plugin-react, vitest (+7 more)
...
Cohesion: 0.25
Nodes (10): Time, Context, Repository, Service, New(), nilIfEmpty(), toResponse(), CreateSchoolRequest (+2 more)
...
Cohesion: 0.05
Nodes (41): main(), DriverWithContext, main(), runMigrations(), splitStatements(), main(), getEnv(), main() (+33 more)
...
Cohesion: 0.26
Nodes (10): Time, Context, Repository, Service, New(), toResponse(), Internal(), CreateStudentRequest (+2 more)
...
Cohesion: 0.15
Nodes (13): FastAPI startup event (launches curriculum_worker.run_forever as background task), PDF extractor (Strategy A: TOC / Strategy B: font heuristics), process_job(), Orchestrates one curriculum-parsing job end to end:    1. Fetch the job row (sub, run_forever(), Two-strategy heading extraction design (TOC-first, font-size/bold fallback), Decision to use plain Redis list consumer instead of Celery task (until Go side pushes Celery-formatted messages), celery==5.4.0 (async task queue dependency, currently unused by curriculum_worker) (+5 more)

### Community 48 - "Impl Plan Doc: (root)"
Cohesion: 0.40
Nodes (8): Conflict(), Forbidden(), New(), NotFound(), NotImplemented(), Unauthorized(), Wrap(), AppError
...
Cohesion: 0.33
Nodes (4): lifespan(), Consumes curriculum-upload jobs off Redis and parses them.  The Go backend pushe, request_shutdown(), FastAPI
...
- **Why does `Internal()` connect `Impl Plan Doc: The Fundamental Dependency Chain` to `Authresponse`, `DB Design Doc: 1 Database Architecture Overview`, `Assessment Dto`, `Ai Careermatchrequest`, `Teacher Dto`, `Jobs Dto`, `Notification Dto`, `Impl Plan Doc: (root)`, `Ministry Dto`, `Pkg Crypto Crypto`, `DB Design Doc: (root)`, `Sync Dto`, `Main Startup Event`?**
  _High betweenness centrality (0.112) - this node is a cross-community bridge._
- **Why does `WriteError()` connect `Pkg Crypto Crypto` to `Region Handler`, `Impl Plan Doc: The Fundamental Dependency Chain`, `School Handler`, `Student Handler`, `Teacher Handler`, `Api Handlers`, `Career Handler`, `Jobs Handler`, `Notification Handler`, `Storage Handler`, `Assessment Handler`, `Auth Handler`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `NotFound()` connect `Impl Plan Doc: (root)` to `Authresponse`, `DB Design Doc: 1 Database Architecture Overview`, `Assessment Dto`, `Ai Careermatchrequest`, `Impl Plan Doc: The Fundamental Dependency Chain`, `Curriculum Dto`, `Teacher Dto`, `Jobs Dto`, `Notification Dto`, `Main Startup Event`?**
