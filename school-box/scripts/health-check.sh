...
if curl -fsS --max-time 5 http://localhost:9090/health >/dev/null 2>&1; then
    ...
    if [ -n "$SECS_SINCE" ]; then
        # SYNC_INTERVAL_MINUTES defaults to 360 (6h) -- flag it as stale at
        # 2x that instead of erroring: a School Box legitimately going
        # offline for a while is expected behavior, not a fault (see
        # agent.go / README.md's "Offline behavior" section), so this is
        # informational, not a FAILED check.
        THRESHOLD_SECS=$(( ${SYNC_INTERVAL_MINUTES:-360} * 60 * 2 ))
        SECS_INT="${SECS_SINCE%.*}"
        if [ "${SECS_INT:-0}" -gt "$THRESHOLD_SECS" ]; then
            echo "  [WARN] last successful sync was ${SECS_INT}s ago (>2x the sync interval) -- check CLOUD_SYNC_ENDPOINT reachability if this box is expected to have internet"
        fi
    fi
